    ...
    <div className="space-y-6">
       ...
       <AnimatePresence>
        {activeAlert && (
            ...
            >
                <LiquidCard className={`border-l-4 p-4 shadow-2xl backdrop-blur-2xl ${
                    activeAlert.type === 'danger' ? 'border-l-red-500 bg-red-50/90' :
...
function SoundButton({ sound, onClick }: { sound: SoundClass, onClick: () => void }) {
    ...
    const bgColors = {
        danger: 'bg-red-50 hover:bg-red-100 border-red-200 text-red-900',
    ...
    return (
        <motion.button
            whileTap={{ scale: 0.95 }}
            onClick={onClick}
            className={`flex flex-col items-center justify-center rounded-2xl border p-4 transition-colors ${bgColors[sound.type]}`}
        >
            <span className="mb-2 text-3xl">{sound.icon}</span>
